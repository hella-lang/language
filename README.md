...
(Draft 0.4: EBNF 0.1 plus the async extension) as a typeset language specification document.
...
| `hella.sty` | LaTeX style file: keyword/built-in macros for Hella's 62 keywords + contextual words (`get`/`set`/`has`/`end`/`convert`/`to`/`extend`/`from`/`initialize`/`Self`), code environments, index/location-marker helpers. |
