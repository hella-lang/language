...
% Source grammar: hella/references/ebnf-0.1.txt (Draft 0.2).
...
\commentary{%
  Change log carried over from the source EBNF (Draft 0.2, Phase 5 fixes):
...
\begin{grammar}
<KEYWORD> ::= \gnewline{}
  \AND{} | \ANY{} | \ARR{} | \ASSERT{} | \BOOL{} | \BREAK{} | \CLASS{}
  ...
  \alt\hspace{-3mm} \FUNCTION{} | \IF{} | \IMPLEMENTS{} | \IMPORT{}
    | \IN{} | \INIT{} | \INT{}
  \alt\hspace{-3mm} \IS{} | \LOOP{} | \MATCH{} | \NOT{} | \NULL{}
    | \OPEN{} | \OPERATOR{}
  \alt\hspace{-3mm} \OR{} | \OUT{} | \OVERRIDE{} | \PRIVATE{}
    | \PUBLIC{} | \REF{} | \RETURN{}
  \alt\hspace{-3mm} \SEALED{} | \STATIC{} | \STRING{} | \STRUCT{}
    | \SUPER{} | \THIS{} | \TRAIT{}
...
\commentary{%
  The EBNF keyword list contains exactly 56 entries:
  \code{and}, \code{any}, \code{arr}, \code{assert}, \code{bool},
  \code{break}, \code{class}, \code{const}, \code{continue},
  \code{debug\_assert}, \code{defer}, \code{distinct}, \code{do},
  ...
  \code{match}, \code{not}, \code{null}, \code{open}, \code{operator},
  \code{or}, \code{out}, \code{override}, \code{private}, \code{public},
  \code{ref}, \code{return}, \code{sealed}, \code{static}, \code{string},
  \code{struct}, \code{super}, \code{this}, \code{trait}, \code{typedef},
  \code{void}, \code{vec}, \code{while}, \code{where}.
...
statement, an assertion, or a block.
...
an optional \code{where} clause, and a body which is either a block
...
<functionDeclaration> ::= <visibilityModifier>? \STATIC{}? \SEALED{}?
  ...
  \gnewline{} `(' <parameterList>? `)' <whereClause>? <functionBody>
...
Each argument is an expression or a \code{name: expression} pair.
...
<externParameter> ::= `...'? <type>? <identifier>
  \alt `...'
\end{grammar}
