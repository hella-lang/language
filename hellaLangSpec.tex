...
% Source grammar: hella/references/ebnf-0.1.txt (Draft 0.5: EBNF 0.1
% plus the async extension: `async' functions, `task<T>', `await',
% `spawn', `scope do ... end', `yield'; plus prefix address-of `&x'
% and dereference `*p' with store-through assignment `*p = v').
...
{\large Draft 0.5 --- EBNF 0.1 plus async, address-of/dereference}}
...
destroys any remaining live \code{own} slot. Structures containing
\code{own} fields are destroyed structurally (per-level member-chain
auto-deref with poisoned moves), and \code{own} may appear in
container elements and in globals, which are destroyed per element
and at program end respectively.
...
\LMHash{}%
Prefix \code{\&x} takes the address of a storage location as a
\code{T*} pointer; prefix \code{*p} loads through a \code{T*} (or
through an \code{own T} handle, which unwraps to its heap object).
In prefix position both are unambiguous (\code{\&} is only infix at
the bitwise-and level, \code{*} only at the multiplicative level).
A dereference is a valid assignment target, so \code{*p = v} stores
through the pointer with the pointee type. Loads respect width and
signedness: an unsigned narrow pointee zero-extends, a signed one
sign-extends.
...
\commentary{%
  ...
  blocking mutexes and channels live in the sync runtime alongside
  them, while genuinely asynchronous I/O, async closures and detached
